...
function playKick(ctx, when, velocity = 1) {
...
function playSnare(ctx, when, velocity = 1) {
...
function playHat(ctx, when, velocity = 1) {
  ...
  const gain = ctx.createGain();
  gain.gain.setValueAtTime(velocity * 0.25, when);
...
function playClap(ctx, when, velocity = 1) {
  const burst = (offset) => {
    ...
    g.gain.setValueAtTime(velocity * 0.35, when + offset);
...
function scheduleHit(ctx, type, when, velocity = 1) {
  switch (type) {
    case 'kick': return playKick(ctx, when, velocity);
    case 'snare': return playSnare(ctx, when, velocity);
    case 'hat': return playHat(ctx, when, velocity);
    case 'clap': return playClap(ctx, when, velocity);
    default: return;
  }
}
...
async function renderToWav({ bpm, pattern, bars = 2 }) {
  ...
  const schedule = (type, when) => {
    switch (type) {
      case 'kick': playKick(oac, when, 1); break;
      case 'snare': playSnare(oac, when, 1); break;
      case 'hat': playHat(oac, when, 1); break;
      case 'clap': playClap(oac, when, 1); break;
    }
  };
  for (let bar = 0; bar < bars; bar++) {
    for (let step = 0; step < STEPS; step++) {
      ...
      TRACKS.forEach((t, row) => {
        if (pattern[row][step]) schedule(t.type, when);
      });
    }
  }
...
function useLocalPatterns() {
  const loadAll = () => {
    try { return JSON.parse(localStorage.getItem('kidbeat_patterns') || '{}'); }
    catch { return {}; }
  };
...
export default function App() {
  ...
  useEffect(() => {
    return () => { if (timerRef.current) window.clearInterval(timerRef.current); try { ctx.close(); } catch {} };
  }, [ctx]);
  ...
  const schedule = (time, step) => {
    TRACKS.forEach((t, row) => {
      if (pattern[row][step]) {
        const g = ctx.createGain();
        g.gain.value = volumes[row];
        g.connect(master);
        const prevDest = ctx.destination;
        ctx.destination = master;
        scheduleHit(ctx, t.type, time, volumes[row]);
        ctx.destination = prevDest;
      }
    });
  };
  ...
  const nextStep = () => {
    ...
    while (nextNoteTimeRef.current < ctx.currentTime + lookahead) {
      ...
      if (swing > 0 && step % 2 === 1) { t += (swing / 100) * (stepDur / 3); }
      schedule(t, step);
  ...
  const randomize = () => {
    setPattern(() => TRACKS.map(()=>Array(STEPS).fill(false)).map((row, rIdx) => row.map(()=> Math.random() < (rIdx===2?0.6:0.3))));
  };
              ...
              <React.Fragment key={t.id}>
                <div>
                  ...
                  <div className="row small" style={{marginTop:6}}>
                    <span>Vol</span>
                    <input className="vol" type="range" min="0" max="1" step="0.05"
                      value={volumes[r]} onChange={(e)=>{
                ...
                {Array.from({length:STEPS}, (_,c) => {
                  ...
                  return (<button key={c} className={cls} onClick={()=>toggleCell(r,c)} aria-label={`${t.label} step ${c+1}`}></button>);
                })}
              </React.Fragment>
            ))}
          </div>
